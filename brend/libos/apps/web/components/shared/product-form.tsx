...
import { Product, categories } from '@/lib/createStoreState'
...
interface ProductFormProps {
  initialData?: Product
  mode: 'new' | 'edit'
  /** Do'kon slug'i — havola va placeholder yo'li uchun. */
  slug: string
  /** Brendga bog'langan mahsulot yaratish/yangilash funksiyalari. */
...
export function ProductForm({ initialData, mode, slug, createProduct, updateProduct, Button, Input }: ProductFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const data = { ...form, images: cleanedImages.length ? cleanedImages : [`/${slug}/placeholder.jpg`] }
      ...
      router.push(`/store/${slug}/admin/products`)
    } catch (e) {
      console.error(e)
      setSaving(false)
    }
        ...
        >
          ...
          <div className="mb-4">
            ...
            <Input
              required
              value={form.sku || ''}
              onChange={(e: React.ChangeEvent<HTMLInputElement>) => set('sku', e.target.value.toUpperCase().replace(/[^A-Z0-9-]/g, ''))}
              placeholder="ASM-001"
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <Input
                required
                value={form.nameUz}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => set('nameUz', e.target.value)}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <Input
                ...
                min="0"
                value={form.price || ''}
                onChange={(e: React.ChangeEvent<HTMLInputElement>) => set('price', Number(e.target.value))}
                placeholder="2500000"
                className="bg-background border-border"
              />
            </div>
