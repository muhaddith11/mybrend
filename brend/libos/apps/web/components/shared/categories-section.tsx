...
import { Product } from '@/lib/createStoreState'

interface CategoriesSectionProps {
  /** Do'kon slug'i — havola va placeholder yo'li uchun. */
  slug: string
  /** Shu do'konning mahsulotlarini olib keluvchi (brendga bog'langan) funksiya. */
  fetchProducts: () => Promise<Product[]>
}

export function CategoriesSection({ slug, fetchProducts }: CategoriesSectionProps) {
  ...
  // mahsulot soni. Link to'g'ri slug bilan /store/<slug>/collection ga ketadi.
      ...
      <div className="container mx-auto px-4 lg:px-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          {categories.map((category, index) => (
            ...
            >
              <Link
                href={`/store/${slug}/collection?category=${encodeURIComponent(category.id)}`}
                className="group relative block aspect-[4/3] overflow-hidden bg-muted"
              >
                {/* Background Image */}
                <Image
                  src={category.image || `/${slug}/placeholder.jpg`}
                  alt={category.name}
                  fill
