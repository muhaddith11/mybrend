...
const MapPicker = dynamic(
  () => import('@/components/asma/map-picker').then((m) => m.MapPicker),
  { ssr: false, loading: () => (
          ...
          >
            ...
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              <Button asChild variant="outline">
                <Link href={`/store/asma/order/${trackId}`}>Buyurtmani kuzatish</Link>
              </Button>
              <Button asChild className="bg-primary text-primary-foreground hover:bg-primary/90">
                <Link href="/store/asma/collection">Xaridni davom ettirish</Link>
              </Button>
            </div>
          </motion.div>
        </div>
