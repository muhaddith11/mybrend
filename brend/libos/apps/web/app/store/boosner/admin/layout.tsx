...
const adminNavItems = [
  { href: '/store/boosner/admin', icon: LayoutDashboard, label: 'Bosh sahifa' },
  { href: '/store/boosner/admin/products', icon: Package, label: 'Mahsulotlar' },
  { href: '/store/boosner/admin/orders', icon: ShoppingCart, label: 'Buyurtmalar' },
  { href: '/store/boosner/admin/lookbook', icon: Image, label: 'Lookbook' },
  { href: '/store/boosner/admin/settings', icon: Settings, label: 'Sozlamalar' },
]
...
export default function AdminLayout({
  ...
  useEffect(() => {
    if (pathname === '/store/boosner/admin/login') { setChecked(true); return }
    const token = localStorage.getItem('boosner_admin_token')
    if (!token) {
      router.replace('/store/boosner/admin/login')
    } else {
      setChecked(true)
    }
  }, [pathname, router])
  ...
  const handleLogout = () => {
    localStorage.removeItem('boosner_admin_token')
    router.replace('/store/boosner/admin/login')
  }

  const isActive = (href: string) =>
    pathname === href || (href !== '/store/boosner/admin' && pathname.startsWith(href))

  if (!checked) return null

  if (pathname === '/store/boosner/admin/login') {
    return <>{children}</>
  }
        ...
        <div className="flex items-center justify-between h-full px-4 lg:px-8">
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2">
              <span className="text-xl font-serif tracking-[0.2em] text-foreground">BOOSNER</span>
